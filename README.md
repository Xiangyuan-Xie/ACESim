...
多旋翼资产支持可选的 `visual_stream` 配置段，可将 MuJoCo 中的机体位姿与旋翼视觉状态通过 ZeroMQ 发布给 UE5 等外部渲染端：

- `params.visual_stream.enabled`
- `params.visual_stream.rate_hz`
- `params.visual_stream.zmq_endpoint`
...
仓库内的 UE5 联调工具位于 `acesim/tools/ue5/`：

- `setup_ubuntu_ue5.sh`
  - 安装 Ubuntu 依赖并尝试拉起 UE5 源码构建链。
- `create_project_scaffold.py`
  - 在 `/tmp/ACESim-unreal/projects/ACESimUE` 生成最小 C++ UE5 项目和 `ACESimBridge` 插件。
- `verify_visual_stream.py`
  - 订阅默认 `tcp://127.0.0.1:5602` 视觉流并打印解码后的样本。

默认联调模式为：

1. ACESim headless 持续执行 `env.step()`。
2. MuJoCo 后端在每步结束后按独立频率发布视觉状态。
3. UE5 中的 `ACESimVehicleSyncComponent` 订阅状态并驱动 `ACESimVehicleActor`。
